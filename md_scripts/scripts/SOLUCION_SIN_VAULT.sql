-- =====================================================
-- SOLUCIÓN SIN NECESIDAD DE VAULT
-- =====================================================
-- Este script configura el trigger para usar valores
-- hardcodeados directamente, sin necesidad de Vault
-- =====================================================

-- Recrear la función del trigger con valores hardcodeados
...
BEGIN
  ...
  IF NEW.driver_id IS NOT NULL AND NEW.type = 'ride_request' THEN
    ...
    IF driver_token IS NOT NULL AND driver_token != '' THEN
      -- Usar valores hardcodeados (más simple, no requiere Vault)
      supabase_url := 'https://wpecvlperiberbmsndlg.supabase.co';
      service_role_key := 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6IndwZWN2bHBlcmliZXJibXNuZGxnIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1MTgzNTQ0OSwiZXhwIjoyMDY3NDExNDQ5fQ.XH4m5a42BTtpeMJCuM_JrHRaOJixszvKEsKhQLDRkwo';
...
BEGIN
  ...
  IF test_driver_id IS NOT NULL THEN
    ...
    RAISE NOTICE '   4. Si NO ves la solicitud, verifica que pg_net esté instalado';
    RAISE NOTICE '';

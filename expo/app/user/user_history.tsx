    ...
    Alert.alert(
      ...
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              console.log('🗑️ Eliminando viajes seleccionados:', selectedIds);
              
              // Eliminar de la base de datos Supabase
              ...
              if (error) {
                ...
                Alert.alert('Error', 'No se pudieron eliminar los viajes de la base de datos');
                return;
              }

              // Solo después de eliminar exitosamente de la BD, eliminar del estado local
              setRides(prev => prev.filter(ride => !selectedIds.includes(ride.id)));
              setSelectedIds([]);
              setSelectMode(false);
              
              Alert.alert('Éxito', `${selectedIds.length} viaje(s) eliminado(s) del historial`);
              console.log('✅ Viajes eliminados exitosamente de Supabase');

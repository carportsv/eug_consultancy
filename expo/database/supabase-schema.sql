...
-- Crear índice único condicional para email (solo para emails no nulos)
...
CREATE TABLE IF NOT EXISTS ride_requests (
    ...
    etaDescription TEXT, -- descripción del ETA
...
-- Políticas de seguridad RLS (Row Level Security) - DESHABILITADAS TEMPORALMENTE
...
-- Políticas para usuarios (compatibles con Firebase Auth) - DESHABILITADAS TEMPORALMENTE
...
-- Políticas para conductores (compatibles con Firebase Auth)
...
CREATE POLICY "Drivers can insert own data" ON drivers FOR INSERT WITH CHECK (true);
CREATE POLICY "Anyone can view available drivers" ON drivers FOR SELECT USING (is_available = true);

-- Políticas para solicitudes de viaje (compatibles con Firebase Auth)
CREATE POLICY "Users can view own rides" ON ride_requests FOR SELECT USING (true);
CREATE POLICY "Users can create own rides" ON ride_requests FOR INSERT WITH CHECK (true);
CREATE POLICY "Drivers can view assigned rides" ON ride_requests FOR SELECT USING (true);
CREATE POLICY "Drivers can update assigned rides" ON ride_requests FOR UPDATE USING (true);

-- Políticas para mensajes (compatibles con Firebase Auth)
...
-- Políticas para configuraciones de usuario (compatibles con Firebase Auth)

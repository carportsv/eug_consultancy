...
class _RequestRideScreenState extends State<RequestRideScreen> {
  ...
  String _buildWhatsAppMessage() {
    ...
    if (origin.isNotEmpty || destination.isNotEmpty) {
      return '${l10n?.whatsappMessageRideHelp ?? "Hola, necesito ayuda con mi reserva"}:\n'
          '${origin.isNotEmpty ? '${l10n?.whatsappLabelOrigin ?? "Origen"}: $origin\n' : ''}'
          '${destination.isNotEmpty ? '${l10n?.whatsappLabelDestination ?? "Destino"}: $destination' : ''}';
    }

    return l10n?.whatsappMessageRideHelp ?? 'Hola, necesito ayuda con mi reserva de taxi';
  }
              ...
              children: [
                ...
                Builder(
                  builder: (context) {
                    final l10n = AppLocalizations.of(context);
                    return Text(
                      l10n?.requestRideCompleteDetails ??
  ...
  String? _validateEmail(String? value) {
    ...
    if (!emailRegex.hasMatch(value)) {
      final l10n = AppLocalizations.of(context);
      return l10n?.commonEnterValidEmail ?? 'Ingrese un email válido';
    }
    return null;
  }

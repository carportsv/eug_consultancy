...
class BookingsCompletedScreen extends StatefulWidget {
  const BookingsCompletedScreen({super.key});

  @override
  State<BookingsCompletedScreen> createState() => _BookingsCompletedScreenState();
}

class _BookingsCompletedScreenState extends State<BookingsCompletedScreen> {
  ...
  Future<void> _loadCompletedRides() async {
    ...
    try {
      ...
      // Cargar viajes con status 'completed'
      ...
      if (kDebugMode) {
        debugPrint('Error cargando viajes completados: $e');
      }
      if (mounted) {
        setState(() {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            ElevatedButton(onPressed: _loadCompletedRides, child: const Text('Reintentar')),
          ],
        ),
      );
    }

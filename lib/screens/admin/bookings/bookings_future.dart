...
class BookingsFutureScreen extends StatefulWidget {
  const BookingsFutureScreen({super.key});

  @override
  State<BookingsFutureScreen> createState() => _BookingsFutureScreenState();
}

class _BookingsFutureScreenState extends State<BookingsFutureScreen> {
  ...
  Future<void> _loadFutureRides() async {
    setState(() {
      _isLoading = true;
      _errorMessage = null;
    });
    ...
    try {
      ...
      // Cargar viajes programados (is_scheduled = true)
      ...
      if (kDebugMode) {
        debugPrint('Error cargando viajes futuros: $e');
      }
      if (mounted) {
        setState(() {
        ...
        child: Center(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              ElevatedButton(onPressed: _loadFutureRides, child: const Text('Reintentar')),
            ],
          ),
        ),
      );
  ...
  Widget _buildTableRow(Map<String, dynamic> ride, bool isTablet) {
    ...
    final scheduledDate = ride['scheduled_date'] != null
      ...
      child: Row(
        children: [
          ...
          Expanded(
            flex: 1,
            child: Text(
              '${DateFormat('MM/dd').format(scheduledDate)} ${scheduledTime.isNotEmpty ? scheduledTime : ''}',
              style: const TextStyle(fontSize: 12),
            ),
          ),
          Expanded(
